...
if (!isset($_SESSION['user_id'])) {
    header("Location: indexru.html"); //redirects if not logged in
    exit();
}

if (isset($_SESSION['name'])) {
    echo "<h2 style='text-align:center; color:#333; margin-top:20px;'>Hi " . htmlspecialchars($_SESSION['name']) . "!</h2>";
}
...
// Handle property add (this page directly inserts into properties1)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_property'])) {
    ...
    $squareMeters= $_POST['squareMeters'] ?? 0;
    ...
    if (!$stmt) {
        ...
        $stmt->bind_param(
            "ssiiisssssi",
            $title, $propertyType, $rent, $bedrooms, $bathrooms,
            $squareMeters, $address, $area, $distance, $description, $landlord_id
        );
        if ($stmt->execute()) {
            $success_msg = "Property added successfully.";
            // clear POST values (so form shows empty on reload)
...
$stmt = $conn->prepare("SELECT property_id, title, property_type, rent, bedrooms, bathrooms, square_meters, address, area, distance, description, created_at
                        FROM properties1
                        WHERE landlord_id = ?
                        ORDER BY created_at DESC");
$stmt->bind_param("i", $landlord_id);
$stmt->execute();
$props_result = $stmt->get_result();
...
<body class="min-h-screen">
    ...
    <div class="container mx-auto px-6 py-8 relative z-10">
        <div class="flex justify-between items-center">
            ...
            <nav class="hidden md:flex space-x-4">
                <!-- Each link redirects to its page -->
                <a href="ownerru.php" class="nav-button px-6 py-3 hover:text-blue-200 transition">Home</a>
                <a href="owner_properties.php" class="nav-button px-6 py-3 hover:text-blue-200 transition">Property management dashboard</a>
                <a href="owner_applications.php" class="nav-button px-6 py-3 hover:text-blue-200 transition">Applications</a>
                <a href="owner_analytics.php" class="nav-button px-6 py-3 hover:text-blue-200 transition">Analytics</a>
                <a href="logout.php" class="nav-button px-6 py-3 hover:text-red-300 transition">Logout</a>
            </nav>
        </div>
    </div>
</header>
        ...
        <section class="py-20 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            <div class="container mx-auto px-6">
                <div class="flex justify-between items-center mb-12">
                    ...
                    <button onclick="openModal('addPropertyModal')" class="clay-button px-8 py-4 text-lg">
    ...
    <form class="card" method="POST" autocomplete="off">
        ...
        <label>Rent (numbers only)
            <input type="number" name="rent" min="0" step="1" required>
        </label>
        ...
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:8px;">
            ...
            <label>Square Meters
                <input type="number" name="squareMeters" min="0" value="30">
            </label>
        </div>
        ...
        <button type="submit" name="add_property" style="background:#2d87f0;color:white;">Add Property</button>
    </form>

    <h2 style="margin-top:20px;">My Listed Properties</h2>
    <div class="grid">
        <?php while ($row = $props_result->fetch_assoc()): ?>
            <div class="prop-card">
                <h3><?= htmlspecialchars($row['title']) ?></h3>
                <div class="muted"><?= htmlspecialchars($row['property_type']) ?> — R<?= number_format($row['rent'], 2) ?></div>
                <p style="margin:8px 0;"><strong><?= (int)$row['bedrooms'] ?> bd</strong> • <?= (int)$row['bathrooms'] ?> ba • <?= (int)$row['square_meters'] ?> m²</p>
                <p class="muted"><?= htmlspecialchars($row['address']) ?> — <?= htmlspecialchars($row['area']) ?></p>
                <p style="margin:8px 0;"><?= nl2br(htmlspecialchars($row['description'])) ?></p>
                <div class="muted" style="font-size:12px;">Listed: <?= htmlspecialchars($row['created_at']) ?></div>
            </div>
        <?php endwhile; ?>
    </div>
</div>
    ...
    <div id="applications" class="tab-content">
        <section class="py-20 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            <div class="container mx-auto px-6">
                <h3 class="text-5xl font-bold text-gray-800 mb-12">Application Management</h3>

                <!-- Application Status Overview -->
                <div class="grid md:grid-cols-4 gap-8 mb-12">
                    <div class="clay-card p-8 text-center border-l-4 border-yellow-400">
                        <i class="fas fa-clock text-5xl text-yellow-500 mb-6"></i>
                        <h4 class="text-2xl font-bold mb-4 text-gray-800">Pending</h4>
                        <p class="text-4xl font-bold text-yellow-500">8</p>
                    </div>
                    <div class="clay-card p-8 text-center border-l-4 border-green-400">
                        <i class="fas fa-check-circle text-5xl text-green-500 mb-6"></i>
                        <h4 class="text-2xl font-bold mb-4 text-gray-800">Approved</h4>
                        <p class="text-4xl font-bold text-green-500">15</p>
                    </div>
                    <div class="clay-card p-8 text-center border-l-4 border-red-400">
                        <i class="fas fa-times-circle text-5xl text-red-500 mb-6"></i>
                        <h4 class="text-2xl font-bold mb-4 text-gray-800">Rejected</h4>
                        <p class="text-4xl font-bold text-red-500">3</p>
                    </div>
                    <div class="clay-card p-8 text-center border-l-4 border-blue-400">
                        <i class="fas fa-file-alt text-5xl text-blue-500 mb-6"></i>
                        <h4 class="text-2xl font-bold mb-4 text-gray-800">Total</h4>
                        <p class="text-4xl font-bold text-blue-500">26</p>
                    </div>
                </div>

                <!-- Recent Applications -->
                <div class="clay-card mb-12">
                    <div class="p-8 border-b border-gray-200">
                        <h4 class="text-2xl font-bold text-gray-800">Recent Applications</h4>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                                <tr>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Applicant</th>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Property</th>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Date Applied</th>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Status</th>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Credit Score</th>
                                    <th class="px-8 py-6 text-left text-sm font-bold text-gray-700 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-8 py-6 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-6">
                                                <div class="text-lg font-bold text-gray-900">Thabo Mthembu</div>
                                                <div class="text-gray-500">thabo.mthembu@student.ukzn.ac.za</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-900 font-medium">Modern Apartment near UKZN</td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-500">2024-01-15</td>
                                    <td class="px-8 py-6 whitespace-nowrap">
                                        <span class="px-4 py-2 text-sm font-bold rounded-full bg-yellow-100 text-yellow-800">Pending</span>
                                    </td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-900 font-bold text-lg">720</td>
                                    <td class="px-8 py-6 whitespace-nowrap text-sm font-medium space-x-3">
                                        <button class="text-green-600 hover:text-green-900 font-semibold">Approve</button>
                                        <button class="text-red-600 hover:text-red-900 font-semibold">Reject</button>
                                        <button class="text-blue-600 hover:text-blue-900 font-semibold">View Details</button>
                                    </td>
                                </tr>
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-8 py-6 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-6">
                                                <div class="text-lg font-bold text-gray-900">Sarah Pillay</div>
                                                <div class="text-gray-500">sarah.pillay@dut.ac.za</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-900 font-medium">Shared House near DUT</td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-500">2024-01-14</td>
                                    <td class="px-8 py-6 whitespace-nowrap">
                                        <span class="px-4 py-2 text-sm font-bold rounded-full bg-green-100 text-green-800">Approved</span>
                                    </td>
                                    <td class="px-8 py-6 whitespace-nowrap text-gray-900 font-bold text-lg">685</td>
                                    <td class="px-8 py-6 whitespace-nowrap text-sm font-medium space-x-3">
                                        <button class="text-blue-600 hover:text-blue-900 font-semibold">View Lease</button>
                                        <button class="text-purple-600 hover:text-purple-900 font-semibold">Message</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="clay-card p-8">
                    <h4 class="text-2xl font-bold mb-8 text-gray-800">Tenant Evaluation Tools</h4>
                    <div class="grid md:grid-cols-3 gap-8">
                        <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-8 border border-blue-200">
                            <i class="fas fa-credit-card text-5xl text-blue-600 mb-6"></i>
                            <h5 class="text-xl font-bold mb-4 text-gray-800">Credit Check</h5>
                            <p class="text-gray-600 mb-6 text-lg">Integration with TransUnion and Experian</p>
                            <button class="clay-button px-6 py-3">Run Check</button>
                        </div>
                        <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl p-8 border border-green-200">
                            <i class="fas fa-file-upload text-5xl text-green-600 mb-6"></i>
                            <h5 class="text-xl font-bold mb-4 text-gray-800">Document Verification</h5>
                            <p class="text-gray-600 mb-6 text-lg">Proof of income and identity verification</p>
                            <button class="clay-button px-6 py-3" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);">Upload Docs</button>
                        </div>
                        <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-8 border border-purple-200">
                            <i class="fas fa-shield-alt text-5xl text-purple-600 mb-6"></i>
                            <h5 class="text-xl font-bold mb-4 text-gray-800">Background Check</h5>
                            <p class="text-gray-600 mb-6 text-lg">Criminal and reference checks</p>
                            <button class="clay-button px-6 py-3" style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%);">Verify</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="analytics" class="tab-content">
        <section class="py-20 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
            <div class="container mx-auto px-6">
                <h3 class="text-5xl font-bold text-gray-800 mb-12">Analytics & Reports</h3>
                ...
                <div class="grid md:grid-cols-4 gap-8 mb-12">
                    <div class="clay-card p-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-lg font-medium">Total Revenue</p>
                                <p class="text-4xl font-bold text-green-600">R2,450,000</p>
                            </div>
                            <i class="fas fa-money-bill-wave text-5xl text-green-500"></i>
                        </div>
                        <div class="mt-4 text-green-600 font-semibold">+12.5% from last month</div>
                    </div>
                    <div class="clay-card p-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-lg font-medium">Occupancy Rate</p>
                                <p class="text-4xl font-bold text-blue-600">92%</p>
                            </div>
                            <i class="fas fa-home text-5xl text-blue-500"></i>
                        </div>
                        <div class="mt-4 text-blue-600 font-semibold">+5.2% from last month</div>
                    </div>
                    <div class="clay-card p-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-lg font-medium">Avg. Response Time</p>
                                <p class="text-4xl font-bold text-purple-600">2.4hrs</p>
                            </div>
                            <i class="fas fa-clock text-5xl text-purple-500"></i>
                        </div>
                        <div class="mt-4 text-purple-600 font-semibold">-0.8hrs from last month</div>
                    </div>
                    <div class="clay-card p-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-lg font-medium">Tenant Retention</p>
                                <p class="text-4xl font-bold text-orange-600">87%</p>
                            </div>
                            <i class="fas fa-users text-5xl text-orange-500"></i>
                        </div>
                        <div class="mt-4 text-orange-600 font-semibold">+3.1% from last month</div>
                    </div>
                </div>
                ...
                <div class="grid lg:grid-cols-2 gap-10 mb-12">
                    ...
                    <div class="clay-card p-8">
                        <h4 class="text-2xl font-bold mb-8 text-gray-800">Monthly Revenue Trends</h4>
                    ...
                    <div class="clay-card p-8">
                        <h4 class="text-2xl font-bold mb-8 text-gray-800">Property Types Distribution</h4>
                        <div style="height: 400px;">
                ...
                <div class="clay-card p-8 mb-12">
                    <h4 class="text-2xl font-bold mb-8 text-gray-800">Tenant Payment Analysis</h4>
                ...
                <!-- Reports Section -->
                <div class="clay-card p-8">
                    <h4 class="text-2xl font-bold mb-8 text-gray-800">Generate Reports</h4>
                    <div class="grid md:grid-cols-3 gap-8">
                        <button class="clay-card p-8 text-center hover:transform hover:scale-105 transition-all duration-300">
                            <i class="fas fa-file-pdf text-5xl mb-6 text-red-500"></i>
                            <div class="text-xl font-bold text-gray-800">Monthly Report</div>
                        </button>
                        <button class="clay-card p-8 text-center hover:transform hover:scale-105 transition-all duration-300">
                            <i class="fas fa-chart-bar text-5xl mb-6 text-green-500"></i>
                            <div class="text-xl font-bold text-gray-800">Performance Analytics</div>
                        </button>
                        <button class="clay-card p-8 text-center hover:transform hover:scale-105 transition-all duration-300">
                            <i class="fas fa-users text-5xl mb-6 text-purple-500"></i>
                            <div class="text-xl font-bold text-gray-800">Tenant Demographics</div>
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div id="loginModal" class="modal">
        <div class="modal-content clay-card max-w-md w-full mx-4 p-10">
            ...
            <form hx-post="handlers/auth.php" 
                  hx-vals='{"action": "login", "csrf_token": "<?php echo generateCSRFToken(); ?>"}'
...
<div id="registerModal" class="modal">
    <div class="modal-content clay-card max-w-lg w-full mx-4 p-10">
        ...
        <form id="registerForm" action="handler.php" method="POST" class="space-y-6">
            <input type="hidden" name="action" value="register"> <!--Hidden action-->
...
<div id="applicationModal" class="modal">
 ...
 <!-- Add Property Modal with php-->
<div id="addPropertyModal" class="modal">
    <div class="modal-content clay-card max-w-3xl w-full mx-4 max-h-screen overflow-y-auto p-10">
        <form action="handler.php" method="POST" class="space-y-8">
    <!-- hidden action -->
    <input type="hidden" name="action" value="add_property">

    <div>
        <label class="block text-lg font-semibold text-gray-700 mb-3">Property Title</label>
        <input type="text" name="title" class="clay-input w-full p-4" placeholder="e.g., Modern 2-bedroom apartment">
    </div>

    <div class="grid md:grid-cols-2 gap-6">
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Property Type</label>
            <select name="propertyType" class="clay-input w-full p-4">
        ...
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Monthly Rent (R)</label>
            <input type="number" name="rent" class="clay-input w-full p-4" placeholder="5500">
        </div>
    </div>

    <div class="grid md:grid-cols-3 gap-6">
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Bedrooms</label>
            <input type="number" name="bedrooms" class="clay-input w-full p-4" placeholder="2">
        </div>
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Bathrooms</label>
            <input type="number" name="bathrooms" class="clay-input w-full p-4" placeholder="1">
        </div>
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Square Meters</label>
            <input type="number" name="squareMeters" class="clay-input w-full p-4" placeholder="45">
        </div>
    </div>

    <div>
        <label class="block text-lg font-semibold text-gray-700 mb-3">Full Address</label>
        <input type="text" name="address" class="clay-input w-full p-4" placeholder="123 Main Street, Durban, 4001">
    </div>

    <div class="grid md:grid-cols-2 gap-6">
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Area</label>
            <select name="area" class="clay-input w-full p-4">
        ...
        <div>
            <label class="block text-lg font-semibold text-gray-700 mb-3">Distance to University (km)</label>
            <input type="number" step="0.1" name="distance" class="clay-input w-full p-4" placeholder="0.5">
        </div>
    </div>

    <div>
        <label class="block text-lg font-semibold text-gray-700 mb-3">Description</label>
        <textarea name="description" class="clay-input w-full p-4 h-32" placeholder="Describe your property..."></textarea>
    </div>

    <div class="flex space-x-6">
        <button type="button" onclick="closeModal('addPropertyModal')" class="flex-1 bg-gray-300 text-gray-700 p-4 rounded-2xl hover:bg-gray-400 transition text-lg font-semibold">Cancel</button>
        <button type="submit" class="flex-1 clay-button p-4 text-lg font-semibold">Add Property</button>
    </div>
</form>
